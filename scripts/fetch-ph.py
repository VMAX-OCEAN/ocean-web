#!/usr/bin/env python3
"""
fetch-ph.py — Download ocean pH data from Copernicus Marine, convert to
raw Float32 binary (.bin), package, upload to GitHub Releases, delete local.

Dataset: global_omi_health_carbon_ph_trend
  - Variables: ph_trend, ph_trend_uncertainty
  - Resolution: 0.25 deg x 0.25 deg (global)
  - Time: annual, 1985 to latest
  - Depth: surface only
  - DOI: 10.48670/moi-00277

Also downloads the surface carbon product for actual pH values:
  MULTIOBS_GLO_BIO_CARBON_SURFACE_REP_015_008
  Dataset: cmems_obs-mob_glo_bgc-car_my_irr-i
  Variables: ph (surface pH)
  - Resolution: 0.25 deg x 0.25 deg (global)
  - Time: monthly, 1985 to latest
...
Usage:
  python scripts/fetch-ph.py --username USER --password PASS
  python scripts/fetch-ph.py --username USER --password PASS --dry-run
"""
...
# pH trend dataset (annual trend map)
PH_TREND_DATASET_ID = "global_omi_health_carbon_ph_trend"
PH_TREND_VARIABLES = ["ph_trend", "ph_trend_uncertainty"]
PH_TREND_DOI = "10.48670/moi-00277"

# Surface carbon dataset (monthly pH values)
PH_SURFACE_DATASET_ID = "cmems_obs-mob_glo_bgc-car_my_irr-i"
PH_SURFACE_VARIABLES = ["ph"]
PH_SURFACE_DOI = "10.48670/moi-00047"

# Output directories
OUTPUT_DIR = Path("public/data/ph")
DOWNLOAD_DIR = Path("downloads/ph")
ZIP_DIR = Path("downloads/ph-zip")

# --- Helpers ---

def log(msg):
    print(f"[fetch-ph] {msg}", flush=True)
...
def convert_netcdf_to_bin(nc_path, variable_name, output_prefix, dataset_label, doi):
    ...
    # Handle datasets with no time dimension (static maps like pH trend)
    ...
    for t in range(n_time):
        ...
        if t == 0 or t == n_time - 1 or (t + 1) % 10 == 0:
            log(f"  Wrote {filename}: shape={arr.shape}, "
                f"min={np.nanmin(arr):.6f}, max={np.nanmax(arr):.6f}, "
                f"size={filepath.stat().st_size} bytes")
...
def package_and_upload(meta_trend, meta_surface):
    """Zip all .bin files + meta.json, upload to GitHub Release."""
    ZIP_DIR.mkdir(parents=True, exist_ok=True)

    # Write meta.json files
    if meta_trend:
        (OUTPUT_DIR / "meta_trend.json").write_text(
            json.dumps(meta_trend, indent=2), encoding="utf-8")
    if meta_surface:
        (OUTPUT_DIR / "meta_surface.json").write_text(
            json.dumps(meta_surface, indent=2), encoding="utf-8")

    # Create zip
    zip_path = ZIP_DIR / "ph-data.zip"
...
def main():
    parser = argparse.ArgumentParser(description="Fetch ocean pH data from Copernicus Marine")
    ...
    parser.add_argument("--end", default="2024-12-31T00:00:00")
    args = parser.parse_args()

    log("=" * 60)
    log("Ocean pH Data Pipeline")
    log("=" * 60)

    meta_trend = None
    meta_surface = None

    # Step 1: Download pH trend dataset
    if not args.surface_only:
        log("")
        log(">>> Step 1: Download pH trend dataset")
        nc_trend = download_dataset(
            dataset_id=PH_TREND_DATASET_ID,
            variables=PH_TREND_VARIABLES,
            output_filename="ph_trend.nc",
            username=args.username, password=args.password,
            dry_run=args.dry_run,
        )
        if nc_trend and not args.dry_run:
            log("")
            log(">>> Step 2: Convert pH trend to .bin")
            meta_trend = convert_netcdf_to_bin(
                nc_path=nc_trend, variable_name="ph_trend",
                output_prefix="ph_trend",
                dataset_label=PH_TREND_DATASET_ID, doi=PH_TREND_DOI,
            )
            convert_netcdf_to_bin(
                nc_path=nc_trend, variable_name="ph_trend_uncertainty",
                output_prefix="ph_trend_unc",
                dataset_label=PH_TREND_DATASET_ID, doi=PH_TREND_DOI,
            )

    # Step 3: Download surface pH dataset
    if not args.trend_only:
        log("")
        log(">>> Step 3: Download surface pH dataset (global)")
        nc_surface = download_dataset(
            dataset_id=PH_SURFACE_DATASET_ID,
            variables=PH_SURFACE_VARIABLES,
            output_filename="ph_surface.nc",
            username=args.username, password=args.password,
        ...
        if nc_surface and not args.dry_run:
            log("")
            log(">>> Step 4: Convert surface pH to .bin")
            meta_surface = convert_netcdf_to_bin(
                nc_path=nc_surface, variable_name="ph",
                output_prefix="ph_surface",
                dataset_label=PH_SURFACE_DATASET_ID, doi=PH_SURFACE_DOI,
            )

    # Step 5: Package and upload
    if not args.dry_run and (meta_trend or meta_surface):
        log("")
        log(">>> Step 5: Package and upload to GitHub Releases")
        package_and_upload(meta_trend, meta_surface)

    # Step 6: Cleanup
    if args.cleanup and not args.dry_run:
        log("")
        log(">>> Step 6: Cleanup local files")
        cleanup_local()
